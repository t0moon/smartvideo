...
市面上绝大多数"广告素材生成"产品，本质仍是**通用视频生成工具**——把"能生成一段视频"当作终点。但品牌广告真正需要的，是从需求理解、分镜、资产准备、场景生成、合成到审核发布的一条完整流水线。现有产品普遍只覆盖流水线的某一个环节，并在价格、资产、合规、模型灵活性、创作终端、Agent 技能六个维度上集中暴露共性痛点。

#### 竞品格局：两类玩家，都没有覆盖企业级生产闭环

| 类别 | 代表产品 | 强项 | 结构性短板（基于 2026-08 公开资料） |
|---|---|---|---|
| **模型厂商自带产品** | 即梦（字节 / Seedance）、可灵（快手 / Kling）、通义万相（阿里 / Wan）| 模型能力强、生成质量高、有免费额度 | 为卖自家算力而生：深度绑定自家模型，缺结构化资产沉淀、缺广告法合规校验、企业级商用按积分隐性成本高（可灵企业会员 ¥10,330 起、团队 559 元/席位/月） |
| **较自由的创作产品** | MiniMax Design / MiniMax Hub | 多 Agent 编排 + Skill 生态 + 本地资产中心 + Review Loop，自由度最高 | 仍是**桌面应用**（macOS / Windows），离开电脑无法创作；模型以 MiniMax 自家为主；**无内建广告法合规审核**；资产为"本地优先"的个人创作资产，非企业级可复用资产库 |

> 此外还有剪映（模板化剪辑辅助，AI 生成能力偏弱）、智谱清影 / 腾讯智影（To C 生成模型 + 剪辑）等，但它们要么 AI 生成依赖模板、要么无企业协作场景，同样只满足单一环节。

这些产品的共同结论是：**能生成视频，不能生产视频**。

#### 六大未被满足的痛点（行业调研佐证）

1. **价格隐性高、被厂商绑定**：基础会员看似便宜（即梦 ¥79/月、可灵 ¥66/月），但企业级商用按积分消耗、商用授权分级复杂、长视频单条算力成本高于行业均值 22%，隐性使用成本高（《2026 AI 视频生成工具真实口碑调研》）。
2. **中间产物不沉淀**：多数工具只交付成片，图片资产、视觉分镜随会话消失；即便即梦有"画布"、可灵企业版有"资产共享"，也是**个人 / 团队创作资产**，未形成可跨项目复用的企业资产库。
3. **缺广告法合规审核**：这是 2026 年最尖锐的痛点——94.1% 广告主已入局 AI 营销，15%–17% 代理商实际遭遇过 AI 内容合规问题，60.55% 从业者将"合规风险"列为 Top 痛点（南都大数据研究院《AI 时代广告主合规增长观察》，2026-08）。但现有**创作工具几乎无内建合规校验**，监管已上升到立法（2025-09《AI 生成内容标识办法》、2026-04 生成式 AI 广告列为六大整治重点）。
4. **模型锁定不灵活**：即梦 / 可灵 / 通义 / MiniMax 均深度绑定自家模型，想换更强或更便宜的模型就得换工具。
5. **创作终端受限**：MiniMax Design 是桌面应用；即梦 / 可灵虽有多端 App，但核心创作仍在 Web / 桌面，离开电脑流程停摆，无法在 IM 里对话式推进。
6. **业务 know-how 难沉淀为 Agent 技能**：即梦 / 可灵 / 通义 / 剪映**完全没有** Skill 体系；MiniMax Design 虽有创作类 Skill（ad-director、mv-creator 等），但是**操作脚本型**技能、且锁定其创作范式，无法把企业自有品牌规范、行业话术、合规知识沉淀为 Agent 可复用的"方法论层"技能。

#### 为什么现有产品难覆盖全链路？

模型厂商的目标是**消耗自家算力、卖 token**；剪辑 / 工具厂商的目标是**卖编辑功能**。即便 MiniMax Design 在"自由创作"上走得更远（多 Agent + Skills + 资产 + Review Loop），它的边界也很清晰：**缺广告法合规、绑定桌面端和自家模型生态、资产偏个人创作而非企业复用**。没有任何一方有动机去建设"流程 + 资产 + 合规 + 多模型 + 多端 + Agent 技能"这套完整基础设施——因为它既不直接卖算力、也不直接卖编辑。这正是 SmartVideo 瞄准的定位空白。
...
| 维度 | 即梦 / 可灵 / 通义 / MiniMax Design | SmartVideo |
|---|---|---|
| 交互方式 | 画布 / Prompt / 桌面指挥中心 | 输入一句话，Agent 自动编排全流程；IM 内对话式推进 |
| 流程管控 | 无结构化管线（MiniMax 有画布流式编排，但仍偏创作） | 7 阶段管线，YAML 可配置 |
