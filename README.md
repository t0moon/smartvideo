...
过去一年国内 AI 视频工具爆发式增长，但它们有一个共同的特征：**工具属性强，流程属性弱**。

| 平台 | 定位 | 亮点 | 局限 |
|---|---|---|---|
| **即梦**（字节）| AI 智能画布 | 多图层编辑、局部重绘、AI 拼图/扩图/抠图、画布拖拽式操作 | 无管线，每个镜头独立操作 |
| **可灵 Kling 3.0**（快手）| 视频生成大模型 | 多模态指令解析、超长视频精准分镜、音画同步 | 仍以 Prompt 为核心交互，无结构化广告流程 |
| **通义万相**（阿里）| 视频/图像生成 | 文生视频、图生视频、风格迁移 | 单次生成，无项目管理和资产沉淀 |
| **智谱清影 / 腾讯混元** | 视频生成 | CogVideoX 开源、混元多模态 | To C 工具，无企业协作场景 |
| **剪映**（字节）| 视频编辑 | 模板化批量生产、AI 功能插件 | AI 生成依赖第三方，非原生 Agent |

这些平台的核心问题：能生成视频，不能生产视频——品牌广告需要从需求理解到审核到发布的一条完整流水线。

#### 为什么现有工具不够用？

1. **Prompt 驱动，无流程**：即梦/可灵是给 Prompt 出视频，改一处要重来全部
2. **无审核机制**：视频生成后人工线下审核，反复改反复生成，品牌合规风险高
3. **无资产沉淀**：上次那条视频的风格很不错——找不回来了，每次都从零描述品牌调性
4. **脱离办公场景**：生成视频下载发飞书等反馈再改，切换 4 个工具

#### SmartVideo 的差异化定位

不是更好的视频生成工具，而是 AI Agent 驱动的品牌视频生产管线。

| 维度 | 即梦 / 可灵 | SmartVideo |
|---|---|---|
| 交互方式 | 画布 / Prompt 工具式操作 | 输入一句话，Agent 自动编排全流程 |
| 流程管控 | 无，全靠人工手动衔接 | 7 阶段管线，YAML 可配置 |
| 审核节点 | 无，出片后线下审 | 4 道 HITL：需求确认/分镜确认/素材确认/成品审核 |
| 资产沉淀 | 无，每次从零开始 | 品牌/人物/分镜/配音自动归档，新项目直接复用 |
| 办公入口 | Web 端 | 飞书内直接创建+审批，纯文本对话式交互 |
